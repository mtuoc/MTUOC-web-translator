...
import net.sf.okapi.connectors.mtuoc.MTUOCConnector;
...
public class Main {
	...
	protected boolean useMTUOC;
	protected String mmtUrl;
	protected String mtuocUrl;
	protected String mmtContext;
	protected String mtuocContext;
	...
	public static void main (String[] originalArgs) {
		...
		try {
			...
			for ( int i=0; i<args.size(); i++ ) {
				...
				else if ( arg.equals("-mtuoc") ) {
					prog.useMTUOC = true;
					// The URL is a mandatory parameter
					prog.mtuocUrl = prog.getArgument(args, ++i);
					// The context is optional
					if ( args.size() > i+1 ) {
						if ( !args.get(i+1).startsWith("-") ) {
							prog.mtuocContext = args.get(++i);
						}
					}
				}
			...
			if ( prog.inputs.isEmpty() ) {
				throw new OkapiException("No input document specified.");
			}
			
			// Process all input files
			prog.process(prog.inputs, args, showTrace);
		}
		catch ( Throwable e ) {
	...
	private void initialize () {		
		...
		extensionsMap.put(".docm", "okf_openxml");
		extensionsMap.put(".pptm", "okf_openxml");
		extensionsMap.put(".xlsm", "okf_openxml");
		extensionsMap.put(".ppsx", "okf_openxml");
		extensionsMap.put(".ppsm", "okf_openxml");
		extensionsMap.put(".dotx", "okf_openxml");
		extensionsMap.put(".dotm", "okf_openxml");
		extensionsMap.put(".potx", "okf_openxml");
		extensionsMap.put(".potm", "okf_openxml");
		extensionsMap.put(".xltx", "okf_openxml");
		extensionsMap.put(".xltm", "okf_openxml");
		extensionsMap.put(".vsdx", "okf_openxml");
		extensionsMap.put(".vsdm", "okf_openxml");
		...
		// We explicitly add only the extensions supported by more than one filter		
		...
		extensionsMap.put(".vtt", "okf_vtt");
		extensionsMap.put(".ttml", "okf_ttml");
		...
		Iterator<FilterConfiguration> configIterator = fcMapper.getAllConfigurations();
		while (configIterator.hasNext()) {
			FilterConfiguration config = configIterator.next();
			if (config != null && config.extensions != null) {
				for (String ext : config.extensions.split(";")) {
					if (!ext.isEmpty() && ext.startsWith(".")) {
						extensionsMap.computeIfAbsent(ext.toLowerCase(Locale.US), e -> config.configId );
					}
				}
			}
		}
	...
	protected void process (List<String> inputs, List<String> args, boolean showTrace) throws URISyntaxException {
		...
		for ( int i=0; i< inputs.size(); i++ ) {
			...
			try {
				processOne(inputs.get(i), args);
			}
			catch ( Throwable e ) {
				displayError(e, showTrace, showTraceHint);
				if (abortOnFailure ) {
					System.exit(1);
				}
				else {
		...
		if ( inputs.size() > 1 ) {
			displayDivider();
			displaySummary(inputs.size(), errorCount, timer);
		}
	}

	protected void processOne (String input, List<String> args) throws URISyntaxException {
		Timer timer = new Timer();
		...
		case CMD_MERGE:
			...
			parameters.setPreserveWhitespace(true);
...
//			return Util.getDirectoryName(Util.getDirectoryName(path));
	...
	private void printUsage () {
		...
		LOGGER.info("      |-pen tmDirectory|-gs configFile|-apertium [configFile]|-mmt url [context]|-mtuoc url [context]");
		...
		LOGGER.info("      |-mm [key]|-pen tmDirectory|-gs configFile|-apertium [configFile]|-mmt url [context]|-mtuoc url [context]");
		LOGGER.info("      |-ms configFile|-tda configFile|-gg configFile|-bi bilingFile|-lingo24 [configFile]");
		...
		LOGGER.info("      [-tt [url]] [-mm [key]] [-pen tmDirectory] [-gs configFile] [-mmt url [context]] [-mtuoc url [context]]");
	...
	private void processQuery () {
		...
		if ( !useGoogleV2 && !useTransToolkit && !useMyMemory
			&& !usePensieve && !useApertium && !useMicrosoft
			&& !useTDA && !useBifile && !useLingo24 && !useMMT && !useMTUOC)
		{
	...
	private IPipelineStep addLeveragingStep () {
		...
		if ( usePensieve ) {
			...
			levParams.setResourceClassName(MTUOCConnector.class.getName());
		}
		...
		throws URISyntaxException
	{
		...
		if ( useGoogleV2 || useTransToolkit || useMyMemory || usePensieve
			|| useApertium || useMicrosoft || useTDA || useBifile
			|| useLingo24 || useMMT || useMTUOC ) {
		...
		// some formats will not merge correctly with simplified placeholders
	...
	private IParameters prepareConnectorParameters (String connectorClassName) {
		...
		if ( connectorClassName.equals(MTUOCConnector.class.getName()) ) {
			net.sf.okapi.connectors.mtuoc.Parameters params
					= new net.sf.okapi.connectors.mtuoc.Parameters();
			params.setUrl(mtuocUrl);
			if (mtuocContext != null) {
				params.setContext(mtuocContext);
			}
			return params;
		}
	...
	private void printScopingReport() {
		...
		// Add leveraging step if requested
		if ( useGoogleV2 || useTransToolkit || useMyMemory || usePensieve
			|| useApertium || useMicrosoft || useTDA || useBifile || useLingo24 || useMMT || useMTUOC) {
			driver.addStep(addLeveragingStep());
